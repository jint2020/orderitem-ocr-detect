...
| `ENABLE_NEW_IR` | `true` | PIR executor. The oneDNN crash happens in PIR's oneDNN instruction path, so `ENABLE_MKLDNN=true` + `ENABLE_NEW_IR=false` may keep oneDNN acceleration while avoiding the bug. |
